...
function get_todos() {
    /*this crates an array of tasks that are inputed*/
    var get_todos = new Array;
    /*this [i;;s tje tasl tjat was saved om tje web brpwser memory*/
    var todos_str = localStorage.getItem('todo');
    /*if the input is not null then JSON.parse will
    communitcate with the web browser to make the task a JS object.*/
    if (todos_str !== null) {
        todos = JSON.parse(todos_str);
    }
    return todos;
}
/* this funciton adds the inputed task to the get_todos function array*/
function add() {
...
function remove() {
    ...
    todos.splice(id, 1);
    localStorage.setItem('todo', JSON.stringify(todos));
    /*this looks in the show() how to display a removed item on the scree */
    show();
