window.alert("Hello, World!");  //window alert message
document.write('Document write message <br>');  //document message

var Sent1 = "This is the beginning of the string <br>"; //creating a variable
document.write(Sent1);
var Sent2 = "and this is the end of the string <br>";  //creating a variable
document.write(Sent2);

var C = (Sent1 + Sent2);  //concatonating a string
document.write(C);

document.write(5+5);  //an expression

